...
public class ClubGrid {
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
		...
		// synchronize the counter object to ensure only one thread at a time checks it
		synchronized (counter){
			while(counter.overCapacity()){ // club has reached its maximum
				counter.wait(); // when the club is at capacity make threads outside the club wait
			}
		}

		// synchronize the entrance object to prevent multiple threads from accessing it
		synchronized(entrance){
			while(entrance.occupied()){ // the entrance is occupied
				entrance.wait(); // when the entrance is occupied do not allow any thread to access it
			}
	...
	public void leaveClub(GridBlock currentBlock,PeopleLocation myLocation)   {
		...
		// Notify the waiting threads waiting that the door is no longer occupied
		synchronized (entrance){
			/* entrance no longer occupied set it to false and notify all threads to enter */
			entrance.release();
			entrance.notifyAll();
		}
        
		// Capacity has changed notify the waiting threads to enter the club
		synchronized (counter){
			counter.notifyAll();
		}
	}
