...
public class Clubgoer extends Thread {
	...
	AtomicBoolean simulationPaused = new AtomicBoolean(false); // boolean flag for start button
	...
	private void checkPause() throws InterruptedException {
		...
		// Synchronize on pausedSimulation for consistent and safe access
		synchronized(ClubSimulation.pausedSimulation) {
			// pause the threads when the paused button is clicked
			while(ClubSimulation.pausedSimulation.get()){
				ClubSimulation.pausedSimulation.wait();
			}
		}
    }
	...
	private void startSim() {
		...
		try {
			// Synchronize the latch for consistent and safe access
			synchronized (ClubSimulation.latch){
				// wait for the latch to open (until the latch's count reaches zero) before starting the simulation
	...
	private void leave() throws InterruptedException {
		// synchronize the club object before using it
		synchronized (club){
