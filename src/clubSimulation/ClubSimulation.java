...
public class ClubSimulation {
	...
	static AtomicBoolean pausedSimulation = new AtomicBoolean(false); // boolean flag for pause button
	
	static CountDownLatch latch; // for threads(patrons) to wait for the start button to be clicked
	...
	public static void setupGUI(int frameX,int frameY,int [] exits) {
		...
		startB.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e)  {
				...
				/* create latch to handle 1 task:
				   1. ensure that all threads(club goers) will wait for the latch to count down before proceeding
				*/
			...
			pauseB.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
                ...
                // synchronize pausedSimulation to ensure memory visibility so that all threads are aware of the changes to pausedSimulation state
				
				synchronized (pausedSimulation){

					/*change the state of the paused simulation to false when it is true
					* to continue the simulation
					*/

					if(pausedSimulation.get()){

						pausedSimulation.set(false);
						// change the button text to 'Pause'
						pauseB.setText("Pause");

						pausedSimulation.notifyAll(); // Notify waiting threads
					
					}

					/*change the state of the paused simulation to true when it is false
					* to pause the simulation
					*/

					else{

						pausedSimulation.set(true);
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// protect the assignment which involves sharing the clubGrid object among multiple threads
		synchronized(Clubgoer.class){
